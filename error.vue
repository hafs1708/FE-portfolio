<template>
    <div class="bg-black w-screen h-screen max-h-screen max-w-screen overflow-hidden relative">
        <a href="/" class="absolute top-10 left-10 md:left-20 text-white text-2xl md:text-4xl font-semibold">{{ url.origin
        }}
        </a>
        ...
        <div class="w-full h-full grid grid-rows-2 md:grid-rows-1 items-center px-20">
            <div class="flex flex-col gap-2 md:gap-6">
                ...
                <div class="text-white error-stack" v-html="error.stack"></div>
                <a href="/" class="btn w-min truncate z-10">Back To Home</a>
            </div>
        </div>

        <!-- <ImagesGhost v-if="error.statusCode = 404 "/> -->
        <ImagesBrokenRobot class="w-[110vw] md:w-[80vw] xl:w-[55vw] absolute -right-28 bottom-10" />
        <ImagesHills
